...
void BlockDCT(vector< CImg<double> >& bloc_image,vector< CImg<double> >& bloc_image_dct, size_t index )
{
    for(int i = 0; i < 8; ++i ) 
    {
        for(int j = 0; j < 8; ++j)
...
CImg<unsigned char> JPEGEncoder(CImg<unsigned char>& image, float quality)
{
    ...
    {
        BlockDCT(bloc_image, bloc_image_dct, index);
    }
    ...
    CImgDisplay main_disp(newImage, "new Image");
