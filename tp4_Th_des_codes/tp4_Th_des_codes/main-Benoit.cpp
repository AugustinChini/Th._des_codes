...
vector< bitset<K> > GSM_decode(vector< bitset<N> > mess_tra)
{
    ...
    for (vector<bitset<N> >::iterator msg_it = mess_tra.begin() ; msg_it != mess_tra.end(); ++msg_it)
    {   
        tmp = paths;
        paths.clear();
    ...
    if( paths.size() > 0 )
    {
        ...
        for (vector<structPath>::iterator it = paths.begin(); it != paths.end(); ++it)
        {
            if( (*it).error < min )
            {
                min = (*it).error;
...
int main()
{
    ...
    mess_tra = GSM_transmission(mess_cod, 4);
